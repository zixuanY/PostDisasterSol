...
class ContrastiveLoss(torch.nn.Module):
    ...
    def forward(self, output1, output2, label):
        ...
        return torch.sum((1-label) * norm + label * max(0, self.margin - torch.sqrt(norm)) ** 2) / output1.size()[0]
...
def similarity(img_url_0="att_faces/training/s1/1.pgm", img_url_1="att_faces/training/s1/1.pgm"):
    img0 = Image.open(img_url_0)
    img1 = Image.open(img_url_1)
    img0 = img0.convert("L")
    img1 = img1.convert("L")
    ...
    x0 = Variable(img0).reshape(1,1,100,100)
    x1 = Variable(img1).reshape(1,1,100,100)
    output1, output2 = net(x0, x1)
    sim = torch.dot(output1.view(-1), output2.view(-1)) * 100
    return sim.item()

if __name__ == "__main__":
    #folder_dataset = dset.ImageFolder(root=Config.training_dir)
    img0 = Image.open("att_faces/training/s1/1.pgm")
    img1 = Image.open('att_faces/training/s1/2.pgm')
    img0 = img0.convert("L")
    img1 = img1.convert("L")
    transform=transforms.Compose([transforms.Resize((100,100)),transforms.ToTensor()])
    img0 = transform(img0)
    ...
    euclidean_distance = F.pairwise_distance(output1, output2)
    concatenated = torch.cat((x0,x1),0)
    imshow(torchvision.utils.make_grid(concatenated),'dissimilarity:%.2f'%(euclidean_distance.item()))
